...
function Dashboard() {
  ...
  const stats = useQuery({
    ...
    queryFn: async () => {
      ...
      return { students: students.count ?? 0, staff: staff.count ?? 0, attendancePct: pct, classes: classes.data ?? [] };
    },
  });
  ...
  const announcements = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("announcements").select("id, title, body, created_at").order("created_at", { ascending: false }).limit(4);
      return data ?? [];
    },
  });
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <StatCard label="Attendance" value={`${stats.data?.attendancePct ?? 0}%`} hint="Today" accent={(stats.data?.attendancePct ?? 0) >= 90 ? "emerald" : "gold"} />
      ...
      <div className="grid grid-cols-12 gap-8">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <Bar dataKey="students" radius={[4, 4, 0, 0]}>
                  {chartData.map((_, i) => <Cell key={i} fill="oklch(0.31 0.13 265)" />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <Card className="col-span-12 lg:col-span-8 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="flex items-center gap-2 text-xs text-muted-foreground">
              <span className="size-2 rounded-full bg-red-500 animate-pulse" />Live GPS
            </span>
          </div>
          <p className="text-sm text-muted-foreground">Bus tracking goes live in Phase 5. Drivers will share GPS; parents will see the bus on a Google Map.</p>
        </Card>
        <Card className="col-span-12 lg:col-span-4 p-6 bg-brand-navy text-white">
          ...
          <p className="text-xs text-white/70 mb-4">End-of-term competency reports for Grade 1–6 will be available in Phase 3.</p>
          <button className="w-full py-2 bg-white text-brand-navy rounded-lg text-xs font-bold">Roadmap</button>
        </Card>
      </div>
    </>
  );
...
function StatCard({ label, value, hint, accent = "navy" }: { label: string; value: string | number; hint?: string; accent?: "navy" | "gold" | "emerald" }) {
  const colorMap = { navy: "text-brand-navy", gold: "text-brand-gold", emerald: "text-brand-emerald" };
  return (
    <Card className="p-6">
